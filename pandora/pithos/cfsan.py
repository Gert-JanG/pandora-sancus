...
class ControlFlowSanitizationPlugin(BasePlugin):
    ...
    1. the current basic block crosses the enclave boundary => WARNING
    2. target to unmeasured AND uninitialized memory => report CRITICAL
    3. target that is tainted AND target may lie inside or outside enclave => report CRITICAL
    4. target that is tainted AND restricted to fully inside enclave => report WARNING

    Criterion 1 checks that a basic block never crosses the enclave boundary such 
    that a part is inside the enclave and another part outside. This indicates improper
    enclave logic as enclaves should always explicitly be left.

    Criterion 2 checks that we don't jump to attacker-controlled memory inside
    the enclave. This should never happen in a sane enclave.

    Criterion 3 checks that we don't jump to attacker-controlled _arbitrary_
    locations. This should never happen in a sane enclave.

    Criterion 4 checks that attacker-controlled jump targets always fall
...
def check_jump(state):
    addr = get_reg_value(state, "ip")
    """
    If jumped to attacker tainted target, dont mind checking implicit exit
    """
    if taint.is_tainted(addr):
        check_tainted_jump(state)
        return
    
    block = state.block()
    block_len = block.size
    """
    Check if current basic block overlaps with the enclave boundary. So a single
    basic block crossing the enclave boundary. As Sancus enclave boundaries are not
    enforced to be crossed by an instruction such as the ENCLU instruction in Intel SGX,
    a single basic block can cross the enclave boundary.
    NOTE: This check does not work when there is a sancus instruction at the boundary as
    this is not recognised by the angr backend and thus a basic block terminates on such 
    an instruction (.size operator does return 0 for sancus instruction as it is not recognized)
    """
    if block_len >= 2 and \
        not buffer_entirely_inside_enclave(state, addr, block_len) \
        and buffer_touches_enclave(state, addr, block_len):
            info = f'Block leaves enclave without explicit jump'
            severity = logging.WARNING
            extra_info="The cause of this warning can also lead to other critical issues (in e.g. PTRSan) \
                because Pandora assumes it is still executing inside the enclave."
            _report_error(state, addr, block_len, False, False, info, severity, extra_info)
    else:
        """
        Only check for 'original' CFSan issues if the jump that occurs is inside the enclave in the first place (if condition evaluated to false)
        """
        check_tainted_jump(state)
...
def check_tainted_jump(state):
    ...
    # of an msp430 instruction (6 bytes)
    target_len = 6
