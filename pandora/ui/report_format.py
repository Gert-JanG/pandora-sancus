...
class LogFormatter(BaseFormatter):
    ...
    def section(self, rip, sym, info, severity):
        ...
        self.logger.log(severity, format_header(f'[{logging.getLevelName(severity)}] [IP={rip:#x} ({sym})] {info}'))
        self.logger.log(severity, '--------------------------------------------------------------------------------')
...
class HTMLFormatter(BaseFormatter):
    ...
    def section(self, rip, sym, desc, severity):
        ...
        badges = self.create_badges({sev: self.badge_styles[sev], f'IP={rip:#x}': 'bg-light text-dark'})
